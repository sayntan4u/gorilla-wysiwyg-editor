  ...
  initEditor() {
    ...
    this.editor.addEventListener("keydown", (event) => {
      ...
      if (event.key == "Backspace") {
        const pageCount = this.editor.querySelectorAll(".go-page").length;
        if (pageCount > 1) {
          var selection = window.getSelection();
          const range =
            selection.rangeCount > 0 ? selection.getRangeAt(0) : null;
          ...
          if (range && range.startContainer) {
            ...
            if (parentElement.innerHTML == "<p><br></p>") {
      ...
      if (event.key == "Enter") {
        ...
        if (range && range.startContainer) {
          const parentElement = this.getPageElement(range.startContainer.parentElement);
